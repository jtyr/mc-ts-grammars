...
module.exports = grammar({
  ...
  rules: {
    ...
    source_file: ($) => seq($.library, optional($.using_list), repeat($.declaration)),

    // library-header = ( attribute-list ) , "library" , compound-identifier , ";"
    library: ($) => seq(optional($.attribute_list), "library", $.compound_identifier, ";"),

    // identifier = [a-zA-Z]([a-zA-Z0-9_]*[a-zA-Z0-9])?
    identifier: (_) => token(seq(letter, optional(repeat(choice(letter, decimal_digit, "_"))))),
    ...
    using: ($) => seq("using", $.compound_identifier, optional(seq("as", $.identifier)), ";"),

    // compound-identifier = IDENTIFIER ( "." , IDENTIFIER )*
    compound_identifier: ($) => seq($.identifier, optional(repeat(seq(".", $.identifier)))),
    ...
    layout_declaration: ($) =>
      seq(optional($.attribute_list), "type", $.identifier, "=", $.inline_layout),
    ...
    //
    // and "overlay"
    layout_kind: (_) => choice("struct", "bits", "enum", "union", "table", "overlay"),

    // layout-body = value-layout | struct-layout | ordinal-layout
    layout_body: ($) => choice($.value_layout, $.struct_layout, $.ordinal_layout),

    // value-layout = "{" , ( value-layout-member , ";" )+ , "}"
    value_layout: ($) => seq("{", repeat1(seq($.value_layout_member, ";")), "}"),

    // value-layout-member = ( attribute-list ) , IDENTIFIER , "=" , constant
    // [NOTE 4]: constant -> constant_allows_bit_ops
    value_layout_member: ($) =>
      seq(optional($.attribute_list), $.identifier, "=", $.constant_allows_bit_ops),

    // struct-layout =  "{" , ( struct-layout-member, ";" )* , "}"
    struct_layout: ($) => seq("{", repeat(seq($.struct_layout_member, ";")), "}"),

    // struct-layout-member = ( attribute-list ) , member-field
    struct_layout_member: ($) => seq(optional($.attribute_list), $.member_field),

    // ordinal-layout =  "{" , ( ordinal-layout-member , ";" )* , "}" ; [NOTE 5]
    ordinal_layout: ($) => seq("{", repeat1(seq($.ordinal_layout_member, ";")), "}"),
    ...
    protocol_event: ($) =>
      seq(
        ...
        $.identifier,
        $.parameter_list,
        optional(seq("error", $.type_constructor)),
      ),

    // parameter-list = "(" , ( type-constructor ) , ")" ; [NOTE 8]
    parameter_list: ($) => seq("(", optional($.type_constructor), ")"),

    // protocol-compose = "compose" , compound-identifier
    //
    // also support attribute-list
    protocol_compose: ($) => seq(optional($.attribute_list), "compose", $.compound_identifier),

    // type-alias-declaration = ( attribute-list ) , "alias" , IDENTIFIER ,  "=" , type-constructor
    type_alias_declaration: ($) =>
      seq(optional($.attribute_list), "alias", $.identifier, "=", $.type_constructor),
    ...
    resource_properties: ($) => seq("properties", "{", repeat(seq($.member_field, ";")), "}", ";"),

    // service-declaration = ( attribute-list ) , "service" , IDENTIFIER , "{" ,
    //                       ( service-member , ";" )* , "}"
    service_declaration: ($) =>
      seq(optional($.attribute_list), "service", $.identifier, "{", repeat($.service_member), "}"),

    // service-member = ( attribute-list ) , member-field ; [NOTE 9]
    // move ; to service-member.
    service_member: ($) => seq(optional($.attribute_list), $.member_field, ";"),

    // member-field = IDENTIFIER , type-constructor
    //
    // It also allow default value as optional.
    member_field: ($) => seq($.identifier, $.type_constructor, optional(seq("=", $.constant))),
    ...
    attribute: ($) =>
      seq("@", $.identifier, optional(seq("(", choice($.constant, $.attribute_args), ")"))),
    ...
    attribute_arg: ($) => seq($.identifier, "=", $.constant),
    ...
    type_constructor: ($) =>
      choice(
        ...
        seq(
          $.layout,
          optional(seq("<", $.layout_parameters, ">")),
          optional(seq(":", $.type_constraints)),
        ),
      ),

    // layout = compound-identifier | inline-layout
    layout: ($) => choice($.builtin_complex_type, $.compound_identifier, $.inline_layout),

    // layout-parameters = layout-parameter | layout-parameter , "," , layout-parameters
    layout_parameters: ($) => seq($.layout_parameter, repeat(seq(",", $.layout_parameter))),
    ...
    layout_parameter: ($) => choice($.type_constructor, $.literal),

    // type-constraints = type-constraint | "<" type-constraint-list ">"
    type_constraints: ($) => choice($.type_constraint, seq("<", $.type_constraint_list, ">")),

    // type-constraint-list = type-constraint | type-constraint , "," , type-constraint-list
    type_constraint_list: ($) => seq($.type_constraint, repeat(seq(",", $.type_constraint))),
    ...
    constant: ($) => choice($.compound_identifier, $.literal),

    // ordinal = NUMERIC-LITERAL

    // literal = STRING-LITERAL | NUMERIC-LITERAL | "true" | "false"
    literal: ($) => choice($.numeric_literal, $.string_literal, $.bool),
    ...
    constant_allows_bit_ops: ($) =>
      choice(
        ...
        seq(
          $.numeric_literal,
          repeat(seq(choice("|", "&"), choice($.numeric_literal, $.compound_identifier))),
        ),
        seq(
          $.compound_identifier,
          repeat(seq(choice("|", "&"), choice($.numeric_literal, $.compound_identifier))),
